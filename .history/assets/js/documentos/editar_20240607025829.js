		...
		success: function (response) {
			...
			.then(responseJson => {
				...
				if (responseJson.status === "success") {
					...
					throw new Error("Erro ao inserir o documento no processo informado no SEI!");
				}
			})
			.catch(err => {
				if (err) {
					swal("Falha!", "Não foi possível inserir o documento no processo informado.", "error");
				} else {
					swal.stopLoading();
					swal.close();
				}
