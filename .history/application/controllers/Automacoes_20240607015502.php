    ...
    if (isset($matches) && is_array($matches)) {
        foreach ($matches[0] as $match) {
           //<textarea name="txaEditor_413" style="display:none;"> 
                $formPostData[$match] = converterParaHtml(parse($editor_base_data,'<textarea name="'.$match.'" style="display:none;">','</textarea>'));
        }
    
        // Adiciona o conteúdo principal ao formulário
        $formPostData[$matches[0][1]] = $conteudo_principal;
    } else {
