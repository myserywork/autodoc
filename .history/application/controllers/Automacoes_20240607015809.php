...
class Automacoes extends CI_Controller {
    ...
    if (isset($matches) && is_array($matches)) {
        ...
        $formPostData[$matches[0][1]] = converterParaHtml(parse($editor_base_data,'<textarea name="'.$matches[0][1].'" style="display:none;">','</textarea>'));
        $formPostData[$matches[0][2]] = $conteudo_principal;
    } else {
