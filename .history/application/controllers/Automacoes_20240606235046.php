...
class Automacoes extends CI_Controller {
    ...
    public function index() {
        ...
        if (isset($pesquisaProcesso['redirectUrl'])) {
            $processo = $this->simplebrowser->get($pesquisaProcesso['redirectUrl']);
        } else {
            echo "Erro ao obter URL de redirecionamento.";
            return;
        }
        ...
        if ($documento_url) {
            ...
            echo "Erro ao obter URL do iframe.";
            return;
        }

        // Parse para obter o link correto
        $parse = $this->dynamicParse($processo_visualizar, ["Nos[0].acoes = '", "linkToNextStep = '"]);
        
        // Visualizar árvore do processo
        if ($parse) {
            ...
            echo "Erro ao obter link da árvore.";
            return;
        }

        // Procurar pelo termo "Termo de Convênio"
        if ($lista) {
            $termo = array_search('Termo de Convênio', array_column($lista, 'text'));
            if ($termo !== false && isset($lista[$termo])) {
                // Criar termo
                $criar_termo = $this->simplebrowser->get('https://sei.agro.gov.br/sei/'. $lista[$termo]->href);
                $url_form = $this->dynamicParse($criar_termo, ['<form id="frmDocumentoCadastro" method="post" onsubmit="return OnSubmitForm();" action="', '<form id="newForm" action="']);
                if ($url_form) {
                    $form = $this->simplebrowser->get('https://sei.agro.gov.br/sei/'. $url_form);
                    echo $form;
                } else {
                    echo "Erro ao obter URL do formulário.";
                }
            } else {
                echo "Termo de Convênio não encontrado.";
            }
        } else {
            echo "Erro ao obter lista de termos.";
        }
    }
    ...
    function dynamicParse($html, $patterns) {
        foreach ($patterns as $pattern) {
            $parse = parse($html, $pattern, '"');
            if ($parse) {
                return str_replace(['<a href="', '"'], '', $parse);
            }
        }
        return null;
    }
