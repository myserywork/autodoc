...
<select class="custom-select ql-insertCustomTags">
    <?php // select all coluns from convenios table, and then generate the SELECT options 
      $conveios = $db->query("SELECT * FROM convenios");
        $conveios->execute();

        while ($row = $conveios->fetch(PDO::FETCH_ASSOC)) {
            echo "<option value='{$row['id']}' data-marker='[convenio_id]' data-title='{$row['nome']}' data-colour='warning'>{$row['nome']}</option>";
        }
